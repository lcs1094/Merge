...
public class Grasshopper_Spwanzone : MonoBehaviour
{
    public bool isDead;
    ...
    void OnTriggerEnter2D(Collider2D col){
        if(col.gameObject.tag == "Monster"){
            ...
            col.gameObject.GetComponent<grasshopper>().Set_Zone(this.transform.position);
        }
    }
    void OnTriggerExit2D(Collider2D col){
        if(col.gameObject.tag == "Monster"){
            //Debug.Log("Zone Out!");
            col.gameObject.GetComponent<grasshopper>().Zone_Out();
        }
    }
    ...
    void Start()
    {
        ...
        Spwan_Monster.GetComponent<grasshopper>().Set_SpwanZone(this.gameObject);
        isDead = false;
    }

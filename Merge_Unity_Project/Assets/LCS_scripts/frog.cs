...
public class frog : MonoBehaviour
{
    ...
    void Die(){
        ...
        My_SpwanZone.GetComponent<Frog_Spwanzone>().Monster_State(isDead);
    ...
    void Update()
    {
        ...
        if(skillon){
            skillon_timer += Time.deltaTime;
            if(skillon_timer > 2.0f){
                skillon = false;
                skillon_timer = 0.0f;
            }
        }
        if(isdetect){
            ...
            if(skill_timer > skill_Active){ // 스킬 발동을 위한 시간이 쿨타임만큼 지나면
                ...
                skill.transform.position = this.transform.position + moveVelocity;    // 스킬의 위치는 몬스터 오브젝트의 위치로 생성
                if(moveVelocity.x > 0){
                    skill.transform.localScale = new Vector3(0.2f,0.1f,1);
                }
                else{
                    skill.transform.localScale = new Vector3(-0.2f,0.1f,1);
                }
